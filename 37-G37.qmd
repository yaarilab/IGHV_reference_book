...
The group of `r paste0('IGHV4-4/IGHV4-59' ,"(", g_group, ")")` includes `r length(grep(g_group,allele_db$func_group,value=T))` functional alleles within it.
...
```{r}
v_calls <- unique(data_frac[grepl(g_group,group),call])
...
```{=html}
 <iframe id="example1" src=" https://peresay.shinyapps.io/absolute_usage_app/?g_group=%22IGHVF8-G36_37_40%22" style="border: none; width: 100%; height: 720px" frameborder="0"></iframe>
```
...
heatmap_alleles(data_frac, g_group, allele_db)%>%
  layout(autosize = F, width = 1000, height = 3000)
```
