...
The term 'Allele clusters' defines alleles that have a degree of germline proximity. The proximity is defined as the hamming distance between the coding region of the alleles' germline sequences. A distance matrix of all alleles' hamming distance is constructed and the hierarchical tree is calculated. The tree leaves are then clustered by 95% similarity which creates the alleles clusters.
...
To illustrate the relation between the International Union of Immunological Societies (IUIS) and IMGT named alleles and the clustered allele clusters we created the figure @fig-clustergroup.
...
## Create your own allele clusters

We created the app below to allow you to create allele clusters based on your own reference.
